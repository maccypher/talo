...
# Bof: put theme names in an object
  ...
  $scope.cssTheme = $store.get('selection') || $scope.themes[0]
  $scope.selectedTheme = $scope.cssTheme
...
# Bof: update theme if new is selected
  $scope.themeUpdate = () ->
    $scope.selectedTheme = $store.get('selection') or $scope.themes[0]
...
# Bof: read LocalStorage for sidebar (edit mode)
  ...
  $scope.editBookmarks = () ->
    ...
    for lsKey, value of localStorage
      unless lsKey in NO_BOOKMARKS
        try
          value = JSON.parse value
        catch e
          console.log 'error by parsing one entry', e, lsKey, value
...
# Bof: fetching the feed Json
  ...
  $scope.fetchFeed = () ->
    ...
    urlGoogle = """
      https://ajax.googleapis.com/ajax/services/feed/load?v=1.0
      &num=25
      &callback=JSON_CALLBACK
      &q=#{encodeURIComponent($scope.feedUrl)}
      &_=123
    """

    request = $http.jsonp urlGoogle

    request.error (data, status) ->
      $scope.feedStatus = status

    request.success (data, status) ->
      return unless data.responseData?
      $scope.rssNotify = false
      $scope.offline = false
      $scope.feedItems = data.responseData.feed.entries
      $scope.feedStatus = status
      latestStoredItem = $store.get 'latestFeed'
      latestItem = new window.Date().getTime()
      firstNewEntry = true
      firstOldEntry = true
